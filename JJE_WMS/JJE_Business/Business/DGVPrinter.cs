...
namespace JJE_Business.Business //AllocationRequest
{
    /// <summary>
    ...
    public class DGVPrinter
    {
        ...
        /// 内容字体型号
        /// </summary>
        private Font cotantfont;

        public Font Cotantfont
        {
            get { return cotantfont; }
            set { cotantfont = value; }
        }
        ...
        private Color contantColor;
        /// <summary>
        /// Allow override of the header cell format object
        /// </summary>


        public Color ContantColor
        {
            get { return contantColor; }
            set { contantColor = value; }
        ...
        public DGVPrinter()
        {
            ...
            subtitlefont = new Font("Tahoma", 10, FontStyle.Bold, GraphicsUnit.Point);
            subtitlecolor = Color.Black;
            footerfont = new Font("Tahoma", 10, FontStyle.Bold, GraphicsUnit.Point);
            footercolor = Color.Black;

            cotantfont = new Font("Tahoma", 10, FontStyle.Bold, GraphicsUnit.Point);
            contantColor = Color.Black;
        ...
        {
            get { return qr_Message; }
            set { qr_Message = value; }
        }
        ...
        private bool PrintPage(Graphics g)
        {
            ...
            while ((printpos + nextrowheight) < staticheight)
            {
                ...
                if (lastrowprinted >= rowstoprint.Count - 1)
                {
                    ...
                    if (list.Count > 0)
                    {
                        DrawContant(g, (float)list[list.Count - 1]);
                        Image image = CreatQR_Barcode(qr_Message, 90, 90);
                        g.DrawImage(image, new Point(600, 220));
                    }
                    return HasMorePages;
                }
                else
        ...
        public void DrawContant(Graphics g, float rectf)
        {
            string contantString = "检验人：                   检验时间：       ";
            //创建页码文本布局
            StringFormat contantFormat = new StringFormat();
            contantFormat.Trimming = StringTrimming.Word;
            contantFormat.FormatFlags = StringFormatFlags.NoWrap | StringFormatFlags.LineLimit | StringFormatFlags.NoClip;
            contantFormat.Alignment = StringAlignment.Near;
            //创建页码的文本格式
            Font contantFont = new Font("宋体", 10, FontStyle.Regular, GraphicsUnit.Point);//打印机点的度量单位
            //创建页码的位置和大小，其中RectangleF(x,y,width,height)
            RectangleF contantRectangle = new RectangleF(100f, rectf + 30, 800F, g.MeasureString(contantString, contantFont).Height);
            g.DrawString(contantString, contantFont, new SolidBrush(Color.Black), contantRectangle, contantFormat);
            //CurrentY += g.MeasureString(contantString, contantFont).Height;
        }
        ...
        /// <returns></returns>
        private Bitmap CreatQR_Barcode(String strMessage, Int32 width, Int32 height)
        {
            Bitmap result = null;
            try
            {
